...
const Home: React.FC = () => {
  return (
    <div className="overflow-hidden bg-slate-50">
      ...
      {/* About Section Teaser - Split Layout FIXED */}
        ...
        <div className="container mx-auto px-6">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 lg:gap-24 items-center">
            <div className="relative order-2 lg:order-1">
              ...
              {/* Quote Card - Positioned relative to avoid overlap issues */}
              <div className="hidden lg:block absolute -right-10 top-3/4 transform -translate-y-1/2 bg-white p-8 rounded-[2rem] shadow-2xl z-20 max-w-sm border border-slate-100">
                <div className="absolute -top-6 left-8 bg-emerald-600 text-white w-12 h-12 rounded-full flex items-center justify-center shadow-lg">
                  <Quote size={20} fill="currentColor" />
                </div>
                <p className="text-slate-600 italic mb-6 leading-relaxed font-medium text-lg mt-4">
                  "Our mission is to nurture the livestock of our nation with world-class biotechnology and integrity."
                </p>
      ...
      <section className="py-20 md:py-32 bg-slate-900 text-white relative overflow-hidden">
        <div className="container mx-auto px-6 relative z-10">
          <div className="text-center mb-16 md:mb-24">
            <span className="text-emerald-400 font-bold tracking-widest uppercase text-xs mb-4 block">Our Workflow</span>
            <h2 className="text-4xl md:text-6xl font-black mb-6 tracking-tighter">Quality in Every Step</h2>
            <p className="text-slate-400 max-w-2xl mx-auto font-medium text-lg">From sourcing to delivery, we maintain the highest standards of safety and efficacy.</p>
          </div>
            ...
            {PROCESS_STEPS.map((step, idx) => {
              ...
              return (
                <div key={idx} className="relative group">
                  <div className="bg-slate-800/50 p-8 rounded-[2rem] border border-slate-700/50 h-full hover:bg-slate-800 transition-all duration-300 hover:-translate-y-2">
                    ...
                    <span className="absolute top-8 right-8 text-5xl font-black text-slate-800 opacity-50 group-hover:opacity-20 transition-opacity">0{step.id}</span>
                  </div>
                  {idx < PROCESS_STEPS.length - 1 && (
                    <div className="hidden md:block absolute top-1/2 -right-4 w-8 h-0.5 bg-slate-700 z-0"></div>
                  )}
                </div>
              );
            })}
      ...
      <section className="py-20 md:py-32 bg-white relative">
        <div className="container mx-auto px-6">
          <div className="text-center mb-16">
            <span className="text-emerald-600 font-bold tracking-widest uppercase text-xs mb-4 block">Success Stories</span>
            <h2 className="text-4xl md:text-6xl font-black text-slate-900 tracking-tighter">Trusted by Professionals</h2>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {TESTIMONIALS.map((t, idx) => (
              <div key={idx} className="bg-slate-50 p-8 rounded-[2.5rem] border border-slate-100 relative group hover:shadow-xl transition-all duration-300">
                <div className="absolute top-8 right-8 opacity-10 group-hover:opacity-20 transition-opacity text-emerald-600">
                  <Quote size={40} />
                </div>
        ...
        <div className="container mx-auto px-6">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-6 md:gap-10">
            {PARTNER_DETAILS.map((p, idx) => (
              <div key={idx} className="group flex flex-col items-center text-center p-8 rounded-[3rem] bg-white hover:bg-emerald-600 transition-colors duration-500 cursor-default shadow-sm hover:shadow-xl">
